import {getTransactions,getAudits,getSkills,OnlyDivPart,pointsRequestByObjectID,getId} from './scripts/queries.js';
import {fixTime,queryFetch,calculateTotal,combineDates,sortArray,formatTimeByDays,calculateDays,sortByDates} from './scripts/util.js';
...
async function populateSecondGraph(arr) {
  ...
  dateArr = arr.map(item => {
    return {
      ...
      pastDays: calculateDays(
        currentTime - new Date(fixTime(item.createdAt)).getTime()
      ),
    };
  });
  // For First Run
  let selectedValue = timePeriod.value;
  let svgArr = arr.filter(
    item => item.pastDays <= timeSelection[selectedValue]
  );
